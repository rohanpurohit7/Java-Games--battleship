...
## Run As A Standalone JavaFX App

You can also build and run the JavaFX game without using Gradle. Install a JDK and the JavaFX SDK, then point these environment variables at them:
...
```text
dist\battleship-command.jar
```

Manual commands, without the batch files:

```powershell
mkdir out\standalone\classes, dist -Force
javac --module-path "$env:JAVAFX_HOME\lib" --add-modules javafx.controls,javafx.graphics -encoding UTF-8 -d out\standalone\classes src\main\java\navalbattle\*.java
jar --create --file dist\battleship-command.jar --main-class navalbattle.BattleshipFxApp -C out\standalone\classes .
java --module-path "$env:JAVAFX_HOME\lib" --add-modules javafx.controls,javafx.graphics -jar dist\battleship-command.jar
```
